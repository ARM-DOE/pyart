...
class read_mdv:
    ...
    def read_a_field(self, fnum, **kwargs):
        ...
        try:
            ...
            sweep_list = zeros([self.master_header['nsweeps'],
                                self.master_header['nrays'],
                                self.master_header['ngates']], dtype=float32)
    ...
    def calc_geometry(self):
        ...
        if self.field_headers[1]['proj_type'] == self.PROJ_POLAR_RADAR:
            self.scan_type = 'ppi'
            az_deg = self.field_headers[1]['grid_miny'] + arange(
                self.master_header['nrays']) * self.field_headers[1]['grid_dy']
    ...
    def append_carts(self):
        ...
        xx = zeros([self.master_header['nsweeps'],
                    self.master_header['nrays'],
                    self.master_header['ngates']], dtype=float32)
        yy = zeros([self.master_header['nsweeps'],
                    self.master_header['nrays'],
                    self.master_header['ngates']], dtype=float32)
        zz = zeros([self.master_header['nsweeps'],
                    self.master_header['nrays'],
                    self.master_header['ngates']], dtype=float32)
