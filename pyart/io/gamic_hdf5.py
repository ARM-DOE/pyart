...
def read_gamic(filename, field_names=None, additional_metadata=None,
    ...
    global_vars = {'platform_type': 'fixed', 'instrument_type': 'radar',
                   ...
                   'references' : 'template_name' , 'instrument_name' : 'site_name', 
    ...
    try:
	scan_time = datetime.datetime.strptime(h5obj['/scan0/how'].attrs['timestamp'],'%Y-%m-%dT%H:%M:%SZ')
    except ValueError:
	scan_time = datetime.datetime.strptime(h5obj['/scan0/how'].attrs['timestamp'],'%Y-%m-%dT%H:%M:%S.000Z')
    ...
    scan_time_se = (scan_time - datetime.datetime(1970,1,1)).total_seconds() * 1e6
    ...
    # create range array
    _range['data'] = np.linspace(range_start+(range_step*range_samples/2.),range_-(range_step*range_samples/2.),bin_count)
    ...
    altitude = _h5_to_dict(h5obj,'/where','height','altitude', filemetadata)
    altitude_agl = None
    ...
    sweep_mode = _h5_to_dict(h5obj,'/what','object','sweep_mode', filemetadata)
    if 'PVOL' in sweep_mode['data']:
	fixed_angle = _h5_to_dict(h5obj,'/scan0/how','elevation','fixed_angle', filemetadata)
    elif 'RHI' in sweep_mode['data']:
	fixed_angle = _h5_to_dict(h5obj,'/scan0/how','azimuth','fixed_angle', filemetadata)
    ...
    target_scan_rate = _h5_to_dict(h5obj,'/scan0/how','scan_speed','target_scan_rate', filemetadata)
    ...
    moments = h5obj['/scan0']
    fields = dict([(moments[k].attrs['moment'], _h5_moments_to_dict(v, filemetadata)) for k, v in moments.iteritems() if 'mom' in k])
...
def _h5_moments_to_dict(h5obj, fmd):
    ...
    d['data'] = d['valid_min'] + h5obj[...] * (d['valid_max'] - d['valid_min'])/div
    return d
