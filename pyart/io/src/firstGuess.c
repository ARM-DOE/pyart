...
void firstGuess(Volume* soundVolume, float missingVal,
     ...
     int numLevs, alt, i, sweepIndex, currIndex, index, numBins, numRays, 
       numSweeps, time, naz;
     unsigned short sond_type = 0; /* sond_type: 0=sounding and 1=VAD */
     unsigned short flag = 0;
     float ua_data[1000][5],f1,f2,f3,f4,f5,f6,f7,f8,f9,data;
     ...
     **					    for U and V least-squares fit */
     ...
     if ((sond_file = fopen(sounding_name, "r"))==NULL) {
	  ...
	  ,sounding_name);
       ...
       if (sounding_name[0]!='W' && sounding_name[0]!='V') {
	 /* Sounding */
	 i=1;
	 ua_data[0][0]=0.0;
	 ua_data[0][1]=0.0;
	 ...
	 while(1) {
	   if (fgets(line, sizeof(line), sond_file)==NULL) break;
           if (sscanf(line, "%f",&data) !=1) {
	     if (line[0]=='H') {
      	       while(fscanf(sond_file, "%f %f %f %f %f %f %f %f %f\n",
		 &f1,&f2,&f3,&f4,&f5,&f6,&f7,&f8,&f9)!=NULL) {
		 if (f1>=0.0&&f4>=0.0&&f5>=0.0) {
                   /*printf("%f \n", f1);*/
		   /* Assign sounding level to array ua_data */
		   ua_data[i][0]=f1; /* Height (m) */
		   ua_data[i][1]=sin(f4*PI/180.0)*f5; /* U REMOVED - (negative)*/
		   ua_data[i][2]=cos(f4*PI/180.0)*f5; /* V REMOVED - (negative)*/
	 ...
	 if (sond_type==1) printf("Number of sounding levels used: %d\n\n",
				  numLevs);
	 else printf("Number of sounding levels used: %d\n\n", numLevs);
	 fclose(sond_file);
       } else {
	 /* VAD */
	 sond_type=1;
	 ...
	 while (1) {
	   if (fgets(line, sizeof(line), sond_file)==NULL) break;
           if (sscanf(line, "%d %f %f %f %f %f %f %d\n",&time,&f1,
              &f2,&f3,&f4,&f5,&f6,&naz) !=1) {
  	      if (time==VAD_time && f6!=99.10 && f4!=0.00) {
		ua_data[i][0]=f2; /* Height (m) */
		ua_data[i][1]=-sin(f4*PI/180.0)*f3; /* U */
		ua_data[i][2]=-cos(f4*PI/180.0)*f3; /* V */
		ua_data[i][3]=(ua_data[i][1]-ua_data[i-1][1])/(ua_data[i][0]-
       				       ua_data[i-1][0]); /* Shear U */
		ua_data[i][4]=(ua_data[i][2]-ua_data[i-1][2])/(ua_data[i][0]-  					      ua_data[i-1][0]); /* Shear V */
		if (fabs(ua_data[i][3])<=MAXSHEAR&&fabs(ua_data[i][4])<=
				     MAXSHEAR) i++;
	      }
	   }
	 ...
	 ua_data[1][3]=0.0;
	 ua_data[1][4]=0.0;
	 fprintf(stderr,"Number of VAD levels used: %d\n\n",numLevs);
	 fclose(sond_file);
       }
