...
def _kdp_estimation_backward_fixed(psidp_in, rcov, pcov_scale, f, f_transposed, h_plus, c1, c2, b1, b2, kdp_th):
    """
...
def _kdp_estimation_forward_fixed(psidp_in, rcov, pcov_scale, f, f_transposed, h_plus, c1, c2, b1, b2, kdp_th):
    """
    Processing one profile of Psidp and estimating Kdp and Phidp
    with the KFE algorithm described in Schneebeli et al, 2014
...
def _kdp_kalman_profile(psidp_in, dr, band = 'X', rcov = 0, pcov = 0):
    ...
    psidp_in = np.ma.filled(psidp_in,np.nan) # Replace missing values with nans
    ...
    psidp = psidp[real_data_ind[0]:real_data_ind[-1]]   
    ...
    psidp_long[nrg + extra_gates: nrg+ 2*extra_gates] = mpsidp + noise
    ...
    #Backward
    kdp_dummy_b2,error_kdp = _kdp_estimation_backward_fixed(psidp, rcov, pcov * scaler, f, f_transposed, h_plus, c1, c2, b1, b2, kdp_th)
    kdp002 = kdp_dummy_b2[extra_gates:nrg + extra_gates]
   
    #Forward
    kdp_dummy_b2,phidp_dummy_b2,error_kdp  = _kdp_estimation_forward_fixed(psidp, rcov, pcov * scaler, f, f_transposed, h_plus, c1, c2, b1, b2, kdp_th)
    kdp002f = kdp_dummy_b2[extra_gates:nrg + extra_gates]
    ...
    for i, sc in enumerate(SCALERS):
        #Backward
        kdp_dummy_b2,error_kdp = _kdp_estimation_backward_fixed(psidp, rcov, pcov * sc, f, f_transposed, h_plus, c1, c2, b1, b2, kdp_th)
        kdp_mat[:,i+1] = kdp_dummy_b2[extra_gates:nrg + extra_gates]
        
        #Forward
        kdp_dummy_b2,phidp_dummy_b2, error_kdp = _kdp_estimation_forward_fixed(psidp, rcov, pcov * sc, f, f_transposed, h_plus, c1, c2, b1, b2, kdp_th)
        kdp_mat[:,i] = kdp_dummy_b2[extra_gates:nrg + extra_gates]
...
def kdp_vulpiani(radar, gatefilter=None, fill_value = None, psidp_field=None,
  ...
  is assumed to be pre-filtered (for ex. with the filter_psidp function)
    ...
    if np.mod(windsize,2):
        print('In the Vulpiani method, the window size should be even')
        print('Using default value, windsize = 10')
        windsize = 10
...
def _kdp_vulpiani_profile(psidp_in,dr, windsize = 10,band = 'X', n_iter = 10, interp = False):
    ...
    for ii in range(0, n_iter):
        ...
        kdp_calc[int(l/2):nn - int(l / 2)] = (psidp[l:nn] - psidp[0:nn-l]) / (2. * l * dr/1000.)
        ...
        kdp_calc[nn - int(l/2):] = (psidp[nn - 1] - psidp[nn - l - 1]) / (2. * l * dr/1000.)             
...
def filter_psidp(radar,psidp_field=None,rhohv_field=None, minsize_seq = 5, 
    ...
    rhohv_field : str, optional
        Cross_ orrelation ratio field. If None, the default field name must
        be specified in the Py-ART configuration file.
