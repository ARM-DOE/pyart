...
main(int argc, char** argv) 
{
    ...
    if (prep || outDZ) DZvolume = radarData->v[DZ_INDEX];
   ZDvolume = radarData->v[ZD_INDEX];
   PHvolume = radarData->v[PH_INDEX];
   RHvolume = radarData->v[RH_INDEX];
   SWvolume = radarData->v[SW_INDEX];
   LRvolume = radarData->v[LR_INDEX];
    ...
    **    field: */
    ...
    radarOut->v[ZD_INDEX]=ZDvolume; /*Added by scollis to put pol vars out*/
    radarOut->v[PH_INDEX]=PHvolume; /*Added by scollis to put pol vars out*/
    radarOut->v[SW_INDEX]=SWvolume; /*Added by scollis to put pol vars out*/
    radarOut->v[RH_INDEX]=RHvolume; /*Added by scollis to put pol vars out*/
    radarOut->v[LR_INDEX]=LRvolume; /*Added by scollis to put pol vars out*/
