...
def grid_ppi_sweeps(radar,
    ...
    for sweep in target_sweeps:
        ...
        radar_ds_tmp["elevation"].attrs = {"long_name": "Elevation angle",
                                           "units": "deg"}
        if radar_ds is None:
...
def grid_rhi_sweeps(radar,
    ...
    for sweep in target_sweeps:
        radar_sw = radar.extract_sweeps([sweep])
        if np.max(radar_sw.azimuth['data']) - np.min(radar_sw.azimuth['data']) > 180.:  # 0 or 180 deg sweep
            if np.any(radar_sw.azimuth['data'] > 180.):
                diff_center = 180.  # 0 to 360 deg
            else:
                diff_center = 0. # -180 to 180
            az_round = np.abs(
                np.mean(radar_sw.azimuth['data'] - 360. * (radar_sw.azimuth['data'] > diff_center).astype(int)) /
        ...
        radar_ds_tmp["azimuth"].attrs = {"long_name": "Azimuth angle",
                                         "units": "deg"}
        if radar_ds is None:
            radar_ds = radar_ds_tmp
        else:
