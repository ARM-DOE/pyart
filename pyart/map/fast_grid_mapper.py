...
def fast_map_to_grid(radars, grid_shape, grid_limits, grid_origin=None,
    ...
    radars : tuple of Radar objects.
        Radar objects which will be mapped to the Cartesian grid. However all
        radars must have the same latitude and longitude of the first one
        They also need to be PPI scans, have full sweeps. Partial or fail
        sweep will create undefined behaviour.
    ...
    interpolation: "nearest" or "linear"
        ...
        'linear' (linear in the elevation coordinate). 'linear' takes fast
        double the time.
    max_range: float
        maximum range, bins with greater radius will be excluded. If `None`,
        it will be set to be the diagonal of the grid region.
    ...
    # for fast access, convert interpolation array into int
    # 0='nearest', 1='linear'
    ...
    if grid_origin is None:
        ...
        grid_origin = (lat, lon)
        # all radar have the same location
        offset = (radars[0].altitude['data'], 0, 0)
    else:
    ...
    if max_range is None:
        max_range = np.sqrt(
            ...
            + (grid_limits[1][1] - grid_limits[1][0]) ** 2
    ...
    # put reflectivity as last field,
    # this will be used in cython side for refl_filter_flag
    ...
    #-90 to +90 with 'elev_res' elevation precision
    sweeps_index_down = np.empty(int(180 / elev_res + 1), dtype=np.int)
    ...
    for i in range(int(180 / elev_res + 1)):
        ...
        if interp_opt == 0:
            #it says down, but it is nearest
            sweeps_index_down[i] = np.abs(sweeps_elev - nominal_elev).argmin()
        else:
    ...
    # 0 to 360 with 'azi_res' azimuth precision
    ray_index = np.empty((len(sweeps), int(360 / azi_res + 1)), dtype=np.int)
    ray_index.fill(-1)
    for isweep, sweep in enumerate(sweeps):
        ...
        rays = sorted([(azi, ray + ini) for ray, azi in
                       enumerate(radar.azimuth['data'][ini:end+1])],
    ...
    #0 to max_range with 'r_res' range precision
    gate_index = np.empty((len(radars), int(max_range / r_res)), dtype=np.int)
    gate_index.fill(-1)
    for iradar, radar in enumerate(radars):
        index = 0
        for i in range(int(max_range / r_res)):
            ...
            for k in range(len(radar.range["data"]) - index):
                # map range down,
                if radar.range["data"][k + index] > nominal_range:
                    break
    ...
    grid_data_simple = cython_fast_map(
        z, y, x, offset[0], offset[1], offset[2], max_range, badval,
        interp_opt, sweeps, sweeps_index_down, sweeps_index_up, ray_index,
        gate_index, radars, fields, refl_filter_flag, max_refl, elev_res,
        azi_res, r_res
        )
