...
def mapplotgrid_3p(grid, fig, **kwargs):
    ...
    Keywords
    ----------
    time_step : the index of the time step to be plotted 
                defaults to 0
    level : Index of the level to be potted in the map window
            	defaults to 0
    field : Field to be plotted
                defaults to REF
    valid_min : Lower range of the colormesh
                Tries to get from attributes, else defaults to -8
    valid_max : Lower range of the colormesh
                Tries to get from attributes, else defaults to 64
    lat_cut, lon_cut: Slicing locations
                defaults to grid origin
    min_lat, max_lat, min_lon, max_lon: ranges for the map window
                defaults to range determined from the data
    debug: verbose state, defaults to false
    
    overplot: set this to run more commands after the map is generated
    ...
    pnyc = pyproj.Proj(proj='lcc',datum='NAD83',lat_0=lat0,lon_0=lon0,x_0=0.0,y_0=0.0)
    ...
    if debug:
        print("X cut element: ",  element_x_cut, " Y cut element: ",  element_y_cut)
    ...
    x_lon_cut, y_lon_cut=m(np.array([lon_cut, lon_cut]), np.array([min_lat, max_lat]))
    x_lat_cut, y_lat_cut=m(np.array([min_lon, max_lon]), np.array([lat_cut, lat_cut]))

    
    pcmesh_map = m.pcolormesh(xd,yd,
                              grid.fields[field]['data'][level,:,:] , 
    ...
    pcolormesh_lon = plt.pcolormesh(grid.axes['y_disp']['data']/1000.0, 
    ...
    pcolormesh_lat = plt.pcolormesh(grid.axes['x_disp']['data']/1000.0, 
                                    grid.axes['z_disp']['data']/1000.0,
    ...
    meshes_colorbar=plt.colorbar(cax=cbax, mappable=pcmesh_map, orientation='horizontal')
    meshes_colorbar.set_label(kwargs.get('cb_label',
                                         grid.fields[field]['long_name'] +'('+\
    ...
    fig.text(0.5, 0.9,'Sliced at '+str(slc_height)+' meters at '+datestr)
