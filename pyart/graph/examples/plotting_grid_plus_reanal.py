...
def reanal_oplot(mapobj, grid, **kwargs):
    ...
    address = ('http://nomads.ncdc.noaa.gov/dods/NCEP_NARR_DAILY/' +
               datestr_s + '/' + datestr + '/narr-a_221_' + datestr +
               '_0000_000')
    ...
    want_ts = abs(data.variables['time'][:] -
                  date2num(grid_date[0],
                           data.variables['time'].units)).argmin()
...
pyart.graph.mapplotgrid_3p(grid, f, vmin=-8, vmax=64, level=5, debug=True,
                           lat_cut=36.5, lon_cut=-98.5, overplot=reanal_oplot)
plt.savefig('test_map_plotting.png', dpi=200)
