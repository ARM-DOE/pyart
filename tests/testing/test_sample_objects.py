...
def test_gaussian_storm_grid_results_correct():
    ...
    # Test Shape
    assert gaussian_storm_2d.fields['reflectivity']['data'].shape == (2, grid_len, grid_len), "Grid shape mismatch"

    # Test Data
    assert gaussian_storm_2d.fields['reflectivity']['data'] is not None, "No data in reflectivity field"
    ...
    assert np.isclose(np.mean(storm_data), expected_mean, atol=5), "Mean value out of expected range"
    assert np.isclose(np.std(storm_data), expected_std, atol=5), "Standard deviation out of expected range"
