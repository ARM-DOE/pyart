...
def test_ray_section_by_section():
    ...
    tst_ph_data = ufraycreator.make_data_array("PH", 0, 10.0)
...
def test_templates_extra():
    ...
    ufraycreator = UFRayCreator(
        radar, FIELD_MAPPING, field_write_order, templates_extra=templates_extra
    )
...
def test_range_start():
    radar = pyart.io.read_uf(pyart.testing.UF_FILE)
    radar.range["meters_to_center_of_first_gate"] += 1500.0
    in_mem = StringIO()
    write_uf(in_mem, radar)
