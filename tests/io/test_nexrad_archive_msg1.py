...
def test_time():
    ...
    assert_almost_equal(radar.time["data"][1], 0.506, 3)
...
def test_instument_parameters():
    ...
    assert_almost_equal(unamb[0], 466000.0, 0)
    assert_almost_equal(unamb[400], 137000.0, 0)
...
def test_field_data_spectrum_width():
    assert_almost_equal(
        radar.fields["spectrum_width"]["data"][367, 16:19], [0.0, 15.5, 16.5]
    )


def test_field_data_velocity():
    assert_almost_equal(
        radar.fields["velocity"]["data"][367, 16:19], [-17.5, -13.0, 7.5]
    )
...
def test_field_data_nearest_neighbor():
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=UserWarning)
        radar = pyart.io.read_nexrad_archive(
            pyart.testing.NEXRAD_ARCHIVE_MSG1_FILE, station="KLOT", linear_interp=False
        )
    assert_almost_equal(
        radar.fields["reflectivity"]["data"][0, 8:16],
...
def test_single_scan():
    radar = pyart.io.read_nexrad_archive(
        pyart.testing.NEXRAD_ARCHIVE_MSG1_FILE, station="KLOT", scans=[0]
    )
    assert radar.nrays == 367
    assert radar.ngates == 460
    assert radar.nsweeps == 1
    assert_almost_equal(
        radar.fields["reflectivity"]["data"][0, 6:10], [19, 26.5, 14.5, 21.5]
    )
