...
def test_azimuth():
    ...
    assert_almost_equal(radar.azimuth["data"][10], 90.0, 0)
...
def test_write_ppi_unicode():
    ...
    with pyart.testing.InTemporaryDirectory():
        ...
        radar.sweep_mode["data"] = np.array(["azimuth_surveillance"], dtype="U")
        pyart.io.write_cfradial(tmpfile, radar)
        ref = netCDF4.Dataset(pyart.testing.CFRADIAL_PPI_FILE)
        dset = netCDF4.Dataset(tmpfile)
        check_dataset_to_ref(dset, ref)
...
def test_delay_field_loading():
    ...
    assert isinstance(
        radar.fields["reflectivity_horizontal"], pyart.lazydict.LazyLoadDict
    )
...
def test_create_ncvar_different_dtype():
    ...
    with pyart.testing.InTemporaryDirectory():
        ...
        dic = {
            "data": np.arange(256, dtype="float32") * 0.5 + 100.0,
            "_Write_as_dtype": "u1",
            "_FillValue": 100.0,
        }
