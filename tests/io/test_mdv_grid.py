...
class Mdv_grid_Tests:
    ...
    def check_target_reflectivity_field(grid):
        ...
        assert np.abs(fdata[0, 0, 0] - 0.0) <= 0.01
        assert np.abs(fdata[0, 60, 50] - 10.0) <= 0.01
        assert np.abs(fdata[0, 110, 90] - 20.0) <= 0.01
        assert np.abs(fdata[0, 160, 130] - 30.0) <= 0.01

        assert np.abs(fdata[1, 0, 0] - 5.0) <= 0.01
        assert np.abs(fdata[1, 60, 50] - 15.0) <= 0.01
        assert np.abs(fdata[1, 110, 90] - 25.0) <= 0.01
        assert np.abs(fdata[1, 160, 130] - 35.0) <= 0.01
    ...
    def test_write_read_target_modified(self):
        ...
        original_grid.metadata["radar_0_alt"] = 300.0
    ...
    def test_raise_warning_to_many_z(self):
        ...
        grid = pyart.testing.make_empty_grid((123, 2, 2), ((0, 1), (0, 1), (0, 1)))
        grid.fields["reflectivity"] = {"data": np.zeros((123, 2, 2))}
        tmpfile = BytesIO()
        assert_warns(UserWarning, pyart.io.write_grid_mdv, tmpfile, grid)
        tmpfile.seek(0)
    ...
    def test_write_types(self):
        ...
        uint8_data = np.ones((2, 2, 2), dtype=np.float32) * 100.0 + 5
        ...
        uint16_data = np.ones((2, 2, 2), dtype=np.float32) * 22.0
        ...
        float32_data = np.ones((2, 2, 2), dtype=np.float32) * 98.0 + 2.0
    ...
    def test_raise_error_on_invalid_dtype(self):
        ...
        grid = pyart.testing.make_empty_grid((2, 2, 2), ((0, 1), (0, 1), (0, 1)))
...
def test_mdv_degree_grid():
    grid = pyart.io.read_grid_mdv(pyart.testing.MDV_GRID_FILE, file_field_names=True)
    ...
    assert_almost_equal(np.ma.max(grid.fields["refl"]["data"]), 20.0, 1)
