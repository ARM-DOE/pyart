...
def test_gate_longitude_latitude():
    ...
    radar.ds["azimuth"] = xr.DataArray(np.array([0, 90, 180, 270, 0]), dims="time")
    radar.ds["elevation"] = xr.DataArray(np.array([0, 0, 0, 0, 10]), dims="time")
    radar.ds["range"] = xr.DataArray(np.array([5, 15, 25, 35]), dims="range")
...
def test_gate_x_y_z():
    ...
    radar.ds["azimuth"] = xr.DataArray(np.array([0, 90, 180, 270, 0]), dims="time")
    radar.ds["elevation"] = xr.DataArray(np.array([0, 0, 0, 0, 10]), dims="time")
    radar.ds["range"] = xr.DataArray(np.array([5, 15, 25, 35, 45]), dims="range")
    ...
    assert_allclose(radar.gate_x.values[3], [-5, -15, -25, -35, -45], atol=1e-14)
    ...
    assert_allclose(radar.gate_y.values[2], [-5, -15, -25, -35, -45], atol=1e-14)
...
def test_init_gate_x_y_z():
    ...
    assert_allclose(
        radar.gate_x.values[1],
        [0.0, 4.3627267, 8.72545, 13.08817, 17.450884],
        atol=1e-14,
    )
    assert_allclose(
        radar.gate_y.values[0],
        [0.0, 249.97852, 499.95685, 749.935, 999.9128],
        atol=1e-14,
    )
    z_sweep0 = np.array([0.0, 3.0, 6.0, 9.0, 13.0])
    assert_allclose(radar.gate_z.values[0], z_sweep0, atol=1e-3)

    # change range, gate_x, y, z are not updated
    radar.ds["range"] = xr.DataArray(np.array([15, 25, 35, 45, 55]), dims="range")
    assert_allclose(
        radar.gate_x.values[1],
        [0.0, 4.3627267, 8.72545, 13.08817, 17.450884],
        atol=1e-14,
