...
def test_composite_z():
    ...
    compz = pyart.retrieve.composite_reflectivity(radar,rhv_filter=False)
    ...
    for sweep in radar.sweep_number['data']:
        if sweep == random_sweep:
            ...
            s_idx = radar.sweep_start_ray_index['data'][sweep]
            e_idx = radar.sweep_end_ray_index['data'][sweep]+1
            z[s_idx:e_idx,:] = np.tile(np.arange(0,z.shape[1])[np.newaxis,:],(e_idx - s_idx,1))
    ...
    assert_array_equal(compz['composite_reflectivity']['data'][random_az,:], np.arange(0,z.shape[1]))