...
mydict=dt_to_dict(netCDF4.num2date(mynetcdf.variables['time'][0], units=mynetcdf.variables['time'].units, calendar=mynetcdf.variables['time'].calendar))
s=0
i1=mynetcdf.variables['sweep_start_ray_index'][s]
i2=mynetcdf.variables['sweep_end_ray_index'][s]
print i2
if i2 > 200:
	rg,ele=meshgrid(mynetcdf.variables['range'],mynetcdf.variables['elevation'][i1:i2])
	az=ones(rg.shape, dtype=float32)*mynetcdf.variables['fixed_angle'][s]
	x,y,z=radar_coords_to_cart(rg,az, ele)
	f=figure()
	pc1=pcolormesh(sqrt(x**2+y**2)/1000.0,z/1000.0,mynetcdf.variables['reflectivity_horizontal'][i1:i2,:], vmax=64, vmin=-8)
	...
	savefig('/data-out/sc8pjt1/cmac_anim/'+fname)
	print "yobbo"
	close(f)
mynetcdf.close()
