...
def test_time():
    ...
    assert radar.time['units'] == 'seconds since 2011-05-23T22:42:59Z'
    assert radar.time['data'].shape == (7920, )
    assert round(radar.time['data'][600]) == 25.0
...
def test_range():
    ...
    assert radar.range['data'].shape == (801, )
...
def test_metadata():
    ...
    assert 'country' in radar.metadata
    assert 'original_container' in radar.metadata
    assert 'project' in radar.metadata
    assert 'state' in radar.metadata
...
def test_altitude():
    ...
    assert round(radar.altitude['data']) == 340.0
...
def test_sweep_number():
    ...
    assert np.all(radar.sweep_number['data'] == range(22))


# sweep_mode attribute
def test_sweep_mode():
    assert 'standard_name' in radar.sweep_mode
    assert radar.sweep_mode['data'].shape == (22, )
...
def test_fixed_angle():
    ...
    assert radar.fixed_angle['data'].shape == (22, )
...
def test_sweep_start_ray_index():
    ...
    assert radar.sweep_start_ray_index['data'].shape == (22, )
...
def test_sweep_end_ray_index():
    ...
    assert radar.sweep_end_ray_index['data'].shape == (22, )
...
def test_azimuth():
    ...
    assert round(radar.azimuth['data'][0]) == 360.0
...
def test_elevation():
    ...
    assert radar.elevation['data'].shape == (7920, )
    assert round(radar.elevation['data'][0], 2) == 0.52
...
# ngates attribute
def test_ngates():
    assert radar.ngates == 801


# nrays attribute
def test_nrays():
    assert radar.nrays == 7920


# nsweeps attribute
def test_nsweeps():
    assert radar.nsweeps == 22
...
def check_field_shape(field):
    assert radar.fields[field]['data'].shape == (7920, 801)
...
def test_field_first_points():
    ...
    fields = {'norm_coherent_power': 1.0,
              'reflectivity_horizontal': -8.0,
              'dp_phase_shift': 90.0,
              'diff_reflectivity': 0.0,
              'mean_doppler_velocity': 0.0,
              'copol_coeff': 1.0,
              'diff_phase': 0.0}
